...
def sdf2mol_list(sdf_file):
    ...
    for index, mol in enumerate(Chem.SDMolSupplier(str(sdf_file),
                                                   sanitize=False,
                                                   removeHs=False)):
...
def pdbqt2sdf(ligand_path):
    ...
    command = 'obabel -ipdbqt {0} -osdf -O {1} ---errorlevel 0'.format(
...
def screeining_result_sdf(receptor_path):
    ...
    radius = 1.5
    sdf_propierties = ['file_name','vina_pose', 'vina_score',
                            'cluster_id', 'clust_lenght','best_pose', 'in_site' ]

    ligands_path = [l for l in receptor_path.iterdir() if l.is_dir()]
    for ligand_path in ligands_path:
        lig_result = pdbqt2sdf(ligand_path)
        lig_result = engine.qt_cluster_a_mol_list(
        lig_result, threashold)
    ...
    write_sdf(sdf_name, screened_molecules[receptor_path.name],
              write_props=sdf_propierties)
...
def compute_and_show_GasteigerCharges(mol):
    ...
    for at in m2.GetAtoms():
        lbl = '%s:%.2f' % (
            at.GetSymbol(), at.GetDoubleProp("_GasteigerCharge"))
...
# =========================================================================== #\n\n ")
    ...
    #        result.write("  {0}\t{1:.1f}\t\t{2:.2f}\t\t{3:.2f}\n".format(b.ligandCode[i[0]], i[1], i[2], i[3]))
            result_file.write("   {0:14s}{1:6.1f}{2:17.2f}{3:16.2f}\n".format(
                ligandCode[i[0]].ljust(4), i[1], i[2], i[3]))
