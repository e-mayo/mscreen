...
def calc_rms_matrix(mol_list):
    ...
    rmsMatrix = np.zeros((len(mol_list), len(mol_list)))
    # print((len(mol_list)))
    nH_mol_list = [ AllChem.RemoveAllHs(m) for m in mol_list]
    mol_list = nH_mol_list
    
    for i in range(len(mol_list)):
        for j in range(i):
            # print(i,j)CalcRMS
            
            AllChem.AlignMol(mol_list[i], mol_list[j])
            AllChem.GetBestRMS(mol_list[i], mol_list[j])
            AllChem.CalcRMS(mol_list[i], mol_list[j])
            rmsMatrix[i, j] = AllChem.GetBestRMS(mol_list[i], mol_list[j])
            # rmsMatrix[i, j] = AllChem.CalcRMS(mol_list[i], mol_list[j])
            rmsMatrix[j, i] = rmsMatrix[i, j]
    return rmsMatrix
...
def get_representative_clust(mol_list, cluster = None, key_energy=None):
    """
    Figure out the representative pose. Attempting the criteria:
        max cluster population
        low cluster energy
    ...
    if len(mol_list) == 0:
        return []
    if not key_energy in mol_list[0].GetPropsAsDict():
        return []
    
    cluster_id = [m.GetProp('cluster_id') for m in mol_list]
    clust_lenght = [m.GetProp('clust_lenght') for m in mol_list]
    energies = [m.GetProp(key_energy) for m in mol_list]
    
    max_cluster_leght = clust_lenght.index(max(clust_lenght))
    min_energy = energies.index(min(energies))
